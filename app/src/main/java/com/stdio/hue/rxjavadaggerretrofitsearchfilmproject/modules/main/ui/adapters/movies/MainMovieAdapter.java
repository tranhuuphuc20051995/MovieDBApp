...
public class MainMovieAdapter extends AbsBindingAdapter<ViewDataBinding> {
    ...
    public MainMovieAdapter(List<Movie> listSlider, List<Movie> listNow, List<Movie> listPopular, List<Movie> listTopRated, List<Movie> listUpcoming, MainMovieAdapterListener listener) {
        super(null);
    ...
    public void updateBinding(ViewDataBinding binding, int position) {
        ...
        if (position == TYPE_SLIDER) {
            ...
            itemBind.rvContent.setAdapter(new MainCommonAdapter(MainCommonAdapter.TYPE_NOW, listNow, listener, MoreMovieActivity.TYPE_NOW));
        } else if (position == TYPE_POPULAR) {
            ItemMainMovieBinding itemBind = (ItemMainMovieBinding) binding;
            itemBind.setTitle("Popular");
            itemBind.rvContent.setLayoutManager(new LinearLayoutManager(context, RecyclerView.HORIZONTAL, false));
            itemBind.rvContent.setAdapter(new MainCommonAdapter(MainCommonAdapter.TYPE_COMMON, listPopular, listener, MoreMovieActivity.TYPE_POPULAR));
        } else if (position == TYPE_TOP) {
            ItemMainMovieBinding itemBind = (ItemMainMovieBinding) binding;
            itemBind.setTitle("Top Rated");
            itemBind.rvContent.setLayoutManager(new LinearLayoutManager(context, RecyclerView.HORIZONTAL, false));
            itemBind.rvContent.setAdapter(new MainCommonAdapter(MainCommonAdapter.TYPE_COMMON, listTopRated, listener, MoreMovieActivity.TYPE_TOP_RATE));
        } else {
            ItemMainMovieBinding itemBind = (ItemMainMovieBinding) binding;
            itemBind.setTitle("Upcoming");
            itemBind.rvContent.setLayoutManager(new LinearLayoutManager(context, RecyclerView.HORIZONTAL, false));
            itemBind.rvContent.setAdapter(new MainCommonAdapter(MainCommonAdapter.TYPE_COMMON, listUpcoming, listener, MoreMovieActivity.TYPE_UPCOMING));
        }
    }
